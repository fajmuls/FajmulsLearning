...
const SessionEngine: React.FC<{
  ...
  category: CategoryType;
    ...
    // State to hold ALL answers: { [questionId]: UserAnswer }
    const [answerMap, setAnswerMap] = useState<Record<string, UserAnswer>>({});
    ...
    }, [timeLeft, isSkdSimulation, isUtbkSimulation]);

    // Reset view specific states when index changes
    useEffect(() => {
    ...
    const handleAnswer = async (option: string) => {
        ...
        // If already answered in Drill mode, prevent changing to see explanation correctly
        if (mode === StudyMode.DRILL && answerMap[currentQ.id]) return;
        ...
        if (currentQ.tkpPoints) {
            ...
            score = isCorrect ? (category === 'UTBK' ? 1000 : 5) : 0;
        }

        // Grading AI for Essay
        let aiEval = '';
        ...
        // Preserve "doubtful" state if it existed
    ...
    const toggleDoubtful = () => {
        ...
        // Only allow marking doubtful if an answer is selected or we want to flag for review
        const newAnswer: UserAnswer = {
            questionId: currentQ.id,
            selectedAnswer: existing?.selectedAnswer || '',
            isCorrect: existing?.isCorrect || false,
            scoreEarned: existing?.scoreEarned || 0,
            timeTakenSeconds: existing?.timeTakenSeconds || 0,
            isOverthinking: existing?.isOverthinking || false,
            isGuessing: existing?.isGuessing || false,
            isDoubtful: !existing?.isDoubtful, // Toggle
    ...
    return (
        <div className="min-h-screen bg-slate-100 dark:bg-slate-900 flex flex-col md:flex-row h-screen overflow-hidden transition-colors">
            
            {/* --- LEFT: MAIN QUESTION AREA --- */}
            <div className="flex-1 flex flex-col h-full overflow-hidden relative order-1">
                
                {/* Header (Mobile Only for Menu Toggle) */}
                <div className="md:hidden bg-white dark:bg-slate-800 p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center shadow-sm z-20">
                    <div className="font-bold text-slate-700 dark:text-white">Soal No. {currentIndex + 1}</div>
                    <div className="flex items-center gap-3">
                        {timeLeft > 0 && (
                            <div className="bg-slate-100 dark:bg-slate-700 px-3 py-1 rounded text-sm font-mono font-bold text-slate-800 dark:text-slate-200">
                                {formatTime(timeLeft)}
                            </div>
                        )}
                        <button onClick={() => setIsMobileGridOpen(!isMobileGridOpen)} className="p-2 bg-indigo-50 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400 rounded">
                            <Grid size={20} />
                        </button>
                    </div>
                </div>
                    ...
                    <div className="max-w-3xl mx-auto pb-24">
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 md:p-8 mb-6">
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                                    {currentQ.metadata?.subtest || 'General'}
                                </span>
                                {currentQ.metadata?.difficulty === 'HOTS' && (
                                    <span className="flex items-center gap-1 text-rose-600 dark:text-rose-400 font-bold text-xs bg-rose-50 dark:bg-rose-900/20 px-2 py-1 rounded-full">
                                        <Zap size={12}/> HOTS
                                    </span>
                                )}
                            </div>
                            ...
                            {/* Question Text */}
                            <div className="prose prose-lg dark:prose-invert max-w-none mb-8 text-slate-800 dark:text-slate-100">
                            ...
                            <div className="space-y-3">
                                {currentQ.type === 'multiple_choice' ? currentQ.options?.map((opt, idx) => {
                                    ...
                                    // Reveal correct/wrong in Practice mode
                                    if (mode !== StudyMode.SIMULATION && currentAns?.selectedAnswer) {
                                    ...
                                    return (
                                        <button 
                                            ...
                                            className={`w-full flex items-start p-4 rounded-xl border-2 transition-all group text-left ${containerClass}`}
                                        >
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm mr-4 transition-colors ${labelClass}`}>
                                                {optLabel}
                                            </div>
                                            <div className="flex-1 pt-1 text-slate-700 dark:text-slate-200 text-base">
                                    ...
                                    <textarea 
                                        className="w-full p-4 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 dark:bg-slate-900 dark:text-white"
                                        placeholder="Ketik jawaban Anda..."
                                        rows={4}
                                        value={currentAns?.selectedAnswer || ''}
                        ...
                        {showExplanation && mode !== StudyMode.SIMULATION && (
                            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-2xl p-6 animate-fade-in-up">
                                <h4 className="font-bold text-emerald-800 dark:text-emerald-300 mb-3 flex items-center gap-2">
                                    <Lightbulb size={20}/> Pembahasan
                                </h4>
                ...
                <div className="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-4 z-10">
                    <div className="max-w-4xl mx-auto flex justify-between items-center">
                        <button 
                            onClick={() => setCurrentIndex(prev => Math.max(0, prev - 1))}
                            disabled={currentIndex === 0}
                            className="flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-50"
                        >
                        ...
                        <button 
                            ...
                            className={`flex items-center gap-2 px-6 py-2 rounded-lg font-bold border transition ${
                                currentAns?.isDoubtful 
                                ? 'bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-400 border-amber-300 dark:border-amber-700' 
                                : 'bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-600'
                            }`}
                        >
                            <input type="checkbox" checked={currentAns?.isDoubtful || false} readOnly className="w-4 h-4 accent-amber-500 cursor-pointer"/>
                            <span>Ragu-ragu</span>
                        </button>
                        ...
                        <button 
                            ...
                            className="flex items-center gap-2 px-6 py-2 rounded-lg font-bold bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200 dark:shadow-none"
                        >
                            <span className="hidden sm:inline">{currentIndex === questions.length - 1 ? 'Selesai' : 'Berikutnya'}</span> <ChevronRight size={20}/>
                        </button>
                    </div>
                </div>
            </div>

            {/* --- RIGHT: NAVIGATION GRID (Sidebar) --- */}
            <div className={`
                ...
                ${isMobileGridOpen ? 'translate-x-0' : 'translate-x-full'} md:translate-x-0 md:relative md:w-80
            `}>
                <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
                    <h3 className="font-bold text-slate-800 dark:text-white">Navigasi Soal</h3>
                    <button onClick={() => setIsMobileGridOpen(false)} className="md:hidden text-slate-500"><XCircle size={24}/></button>
                </div>
                ...
                {timeLeft > 0 && (
                    <div className="p-6 pb-0 hidden md:block">
                         <div className="bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 rounded-xl p-4 text-center">
                             <div className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-1">Sisa Waktu</div>
                             <div className="text-3xl font-mono font-bold text-slate-800 dark:text-white">{formatTime(timeLeft)}</div>
                         </div>
                    </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    <div className="grid grid-cols-5 gap-2">
                        {questions.map((q, i) => {
                            ...
                            let bgClass = "bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300"; // Default
                            if (ans?.selectedAnswer) bgClass = "bg-emerald-500 border-emerald-500 text-white"; // Answered
                            if (ans?.isDoubtful) bgClass = "bg-amber-400 border-amber-400 text-white"; // Doubtful override
                            if (isActive) bgClass += " ring-2 ring-indigo-500 ring-offset-2 dark:ring-offset-slate-800"; // Active focus

                            return (
                                <button
                                    key={i}
                                    onClick={() => { setCurrentIndex(i); setIsMobileGridOpen(false); }}
                                    className={`aspect-square rounded-lg border flex items-center justify-center font-bold text-sm transition-all ${bgClass}`}
                                >
                                    {i + 1}
                                </button>
                            );
                ...
                {/* Legend */}
                <div className="p-6 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs text-slate-500 dark:text-slate-400 space-y-2">
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-emerald-500"></div> Sudah dijawab</div>
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-amber-400"></div> Ragu-ragu</div>
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-white border border-slate-300"></div> Belum dijawab</div>
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded border-2 border-indigo-500"></div> Posisi saat ini</div>
                </div>

                <div className="p-4 border-t border-slate-200 dark:border-slate-700 md:hidden">
                    <button onClick={finishExam} className="w-full py-3 bg-rose-600 text-white rounded-xl font-bold">Hentikan Ujian</button>
                </div>
            </div>

            {/* Backdrop for Mobile */}
            {isMobileGridOpen && (
                <div className="fixed inset-0 bg-black/50 z-20 md:hidden" onClick={() => setIsMobileGridOpen(false)}></div>
            )}
...
// ... (Other components remain defined below, ensuring they use full text rendering for History)
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="space-y-6">
                        {item.questions.map((q, i) => {
                            ...
                            return (
                                <div key={i} className="p-6 border border-slate-100 dark:border-slate-700 rounded-xl bg-slate-50/50 dark:bg-slate-800/50">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="text-sm font-bold bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-3 py-1 rounded">Soal {i+1}</div>
                                        {ans?.isCorrect ? 
                                            <span className="text-emerald-600 flex items-center text-sm font-bold"><CheckCircle size={16} className="mr-1"/> Benar</span> : 
                                            <span className="text-rose-500 flex items-center text-sm font-bold"><XCircle size={16} className="mr-1"/> Salah</span>
                                        }
                                    </div>
                                    <div className="mb-6 text-slate-800 dark:text-slate-200 text-lg leading-relaxed">
                                        <SimpleMarkdown text={q.content} />
                                    </div>
                                    
                                    <div className="grid md:grid-cols-2 gap-4 mb-6">
                                        <div className={`p-4 rounded-xl border ${ans?.isCorrect ? 'bg-emerald-50 border-emerald-100 dark:bg-emerald-900/20 dark:border-emerald-800' : 'bg-rose-50 border-rose-100 dark:bg-rose-900/20 dark:border-rose-800'}`}>
                                            <div className="text-xs uppercase font-bold mb-1 opacity-70">Jawaban Kamu</div>
                                            <div className="font-bold">{ans?.selectedAnswer || '-'}</div>
                                        </div>
                                        {!ans?.isCorrect && (
                                            <div className="p-4 rounded-xl border bg-indigo-50 border-indigo-100 dark:bg-indigo-900/20 dark:border-indigo-800">
                                                <div className="text-xs uppercase font-bold mb-1 opacity-70">Kunci Jawaban</div>
                                                <div className="font-bold">{q.correctAnswer}</div>
                                            </div>
                                        )}
                                    </div>

                                    <div className="mt-4 bg-white dark:bg-slate-700 p-5 rounded-xl border border-slate-200 dark:border-slate-600">
                                        <h4 className="font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2"><Lightbulb size={18} className="text-amber-500"/> Pembahasan Detail:</h4>
                        ...
                        {questions.map((q, i) => {
                            ...
                            return (
                                <div key={i} className="p-4 border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-750 transition">
                                    ...
                                    {!isCorrect && q.type === 'multiple_choice' && <p className="text-xs text-rose-500 mt-2 font-bold">Jawaban: {ans?.selectedAnswer || '-'} (Kunci: {q.correctAnswer})</p>}
...
// ... (Other components ColorBlindTest, HistoryView, etc. stay same, just ensure they are included)

// ColorBlindPlate and ColorBlindTest (Re-declaring here as they were in previous App.tsx structure)
...
// ... (Rest of components: Dashboard, App are below - Dashboard unchanged in structure, App structure preserved)
...
const Dashboard: React.FC<{
  ...
  // Re-pasting Dashboard content for completeness as it was truncated in previous turn's thought process but required for the file
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
        ...
        reader.onload = (ev) => {
             ...
             setTimeout(() => {
                 if (setGeneralInput) {
                     setGeneralInput({ type: 'pdf', content: rawBase64, title: file.name, lengthPreference: materialLength, difficultyPreference: questionDiff });
                 }
                 setPdfLoading(false);
             }, 800);
        };
    ...
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm mb-6 animate-fade-in-up">
            ...
            <div className="mt-6 border-t border-slate-100 dark:border-slate-700 pt-6 animate-fade-in-up">
                <div className="flex flex-col sm:flex-row gap-4 items-end">
                    <div className="flex-1 w-full">
                        ...
                        <select value={practiceCount} onChange={(e) => setPracticeCount(Number(e.target.value))} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl font-medium dark:text-white">
                    ...
                    <div className="flex-1 w-full">
                        ...
                        <select value={practiceDifficulty} onChange={(e) => setPracticeDifficulty(e.target.value)} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl font-medium dark:text-white">
                    ...
                    <button onClick={() => onStartSession(StudyMode.PRACTICE, practiceDifficulty, practiceCount)} className="w-full sm:w-auto px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 dark:shadow-none transition flex items-center justify-center gap-2">
        ...
        {category === 'INTERVIEW' && selectedSubtest && (
            <div className="mt-6 border-t border-slate-100 dark:border-slate-700 pt-4 animate-fade-in-up">
                <button onClick={() => onStartSession(StudyMode.DRILL)} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 dark:shadow-none transition flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-6 transition-colors duration-300">
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            <button onClick={onBack} className="flex items-center text-slate-500 dark:text-slate-400 hover:text-indigo-600 font-medium"><ArrowLeft size={18} className="mr-2" /> Kembali</button>
            <div className="flex items-center gap-3">
                {onToggleDarkMode && (<button onClick={onToggleDarkMode} className="p-2 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-indigo-600 transition">{isDarkMode ? <Sun size={16}/> : <Moon size={16}/>}</button>)}
                <span className="text-xs font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-full flex items-center gap-1"><UserIcon size={12}/> {username}</span>
            </div>
        </div>
        <div className="flex justify-between items-end mb-8">
          <div><span className="text-sm font-bold tracking-wider text-indigo-600 uppercase mb-1 block">{category} Dashboard</span><h1 className="text-3xl font-bold text-slate-900 dark:text-white">Pusat Kendali Belajar</h1></div>
        </div>
        {loading ? (
            <div className="flex flex-col items-center justify-center py-20"><Loader2 className="animate-spin text-indigo-600 w-10 h-10 mb-4" /><p className="text-slate-500 font-medium">AI sedang memproses materi...</p><div className="max-w-md w-full mt-8 space-y-4"><CardSkeleton /></div></div>
        ) : (
            <>
                {category === 'SKRIPSI' && (
                     <div className="animate-fade-in-up max-w-2xl mx-auto">
                         <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm mb-6">
                             ...
                             <textarea className="w-full p-4 border border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl mb-6 focus:ring-2 focus:ring-indigo-500 h-32" placeholder="Contoh: Pengaruh Kecerdasan Buatan terhadap..." value={inputText} onChange={e => setInputText(e.target.value)}/>
                             <div className="grid grid-cols-2 gap-3">
                                 <button onClick={() => onStartSkripsiSession && onStartSkripsiSession('TITLE_IDEAS', inputText)} className="p-4 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 rounded-xl font-bold hover:bg-indigo-100 dark:hover:bg-indigo-900/40 flex flex-col items-center gap-2 border border-indigo-100 dark:border-indigo-800"><Brain size={24}/> Ide Judul</button>
                                 <button onClick={() => onStartSkripsiSession && onStartSkripsiSession('OUTLINE', inputText)} className="p-4 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 rounded-xl font-bold hover:bg-emerald-100 dark:hover:bg-emerald-900/40 flex flex-col items-center gap-2 border border-emerald-100 dark:border-emerald-800"><FileText size={24}/> Buat Outline</button>
                                 <button onClick={() => onStartSkripsiSession && onStartSkripsiSession('METHODOLOGY', inputText)} className="p-4 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 rounded-xl font-bold hover:bg-amber-100 dark:hover:bg-amber-900/40 flex flex-col items-center gap-2 border border-amber-100 dark:border-amber-800"><Search size={24}/> Cek Metodologi</button>
                                 <button onClick={() => onStartSkripsiSession && onStartSkripsiSession('PARAPHRASE', inputText)} className="p-4 bg-rose-50 dark:bg-rose-900/20 text-rose-700 dark:text-rose-300 rounded-xl font-bold hover:bg-rose-100 dark:hover:bg-rose-900/40 flex flex-col items-center gap-2 border border-rose-100 dark:border-rose-800"><FileSearch size={24}/> Parafrase Teks</button>
                             </div>
                         </div>
                     </div>
                )}
                        ...
                        {!isReadingMaterial ? (
                            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
                                ...
                                <div className="flex gap-2 mb-4">
                                    <input type="text" className="flex-1 p-3 border border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-xl" placeholder={pdfName ? `PDF Terpilih: ${pdfName}` : "Contoh: Perang Dunia II..."} value={inputText} disabled={!!pdfName} onChange={e => setInputText(e.target.value)}/>
                                    <input type="file" ref={fileInputRef} accept="application/pdf" className="hidden" onChange={handleFileChange}/>
                                    <button className={`p-3 rounded-xl transition flex items-center gap-2 ${pdfName ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200'}`} title="Upload PDF" onClick={() => fileInputRef.current?.click()}><FileText size={20}/><span className="hidden sm:inline font-bold text-sm">Upload PDF</span></button>
                                </div>
                                {pdfName && (<div className="mb-4 text-sm bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 px-4 py-2 rounded-lg flex items-center justify-between animate-fade-in-up border border-emerald-100 dark:border-emerald-800"><div className="flex items-center gap-2"><CheckCircle size={16}/> <span>File siap: <b>{pdfName}</b></span></div><button onClick={() => { setPdfName(null); setGeneralInput && setGeneralInput(null); if (fileInputRef.current) fileInputRef.current.value = ''; }} className="text-slate-400 hover:text-rose-500" title="Hapus PDF"><Trash2 size={16}/></button></div>)}
                                {pdfLoading ? (<div className="mb-6 space-y-2"><div className="flex items-center gap-2 text-sm text-slate-500 mb-2"><Loader2 size={14} className="animate-spin"/> Memuat Preview PDF...</div><SkeletonLoader height="h-[300px]" className="rounded-xl" /></div>) : generalInput?.type === 'pdf' && generalInput.content && (<div className="mb-6 animate-fade-in-up"><label className="text-xs font-bold text-slate-500 mb-2 block">Preview PDF:</label><embed src={`data:application/pdf;base64,${generalInput.content}`} className="w-full h-[300px] rounded-xl border border-slate-200 dark:border-slate-700" type="application/pdf" /></div>)}
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                    <div><label className="text-xs font-bold text-slate-500 mb-1 block">Panjang Materi</label><select value={materialLength} onChange={(e) => setMaterialLength(e.target.value as MaterialLength)} className="w-full p-3 bg-slate-50 dark:bg-slate-700 dark:text-white border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-medium"><option value="SHORT">Ringkas</option><option value="MEDIUM">Standar</option><option value="LONG">Mendalam</option></select></div>
                                    <div><label className="text-xs font-bold text-slate-500 mb-1 block">Kesulitan Soal</label><select value={questionDiff} onChange={(e) => setQuestionDiff(e.target.value as QuestionDifficulty)} className="w-full p-3 bg-slate-50 dark:bg-slate-700 dark:text-white border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-medium"><option value="EASY">Mudah</option><option value="MEDIUM">Sedang</option><option value="HARD">Sulit</option></select></div>
                                </div>
                                <button onClick={handleInputSubmit} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 dark:shadow-none hover:bg-indigo-700 transition">Proses & Baca Materi</button>
                            </div>
                        ) : (
                            <div className="animate-fade-in-up">
                                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 mb-6">
                                    <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-100 dark:border-slate-700"><h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><BookOpen size={20}/> Materi Belajar</h3><button onClick={onResetMaterial} className="text-sm text-rose-500 hover:underline">Ganti Materi</button></div>
                                    {generalInput?.type === 'pdf' && generalInput.content && (<div className="mb-6"><embed src={`data:application/pdf;base64,${generalInput.content}`} className="w-full h-[500px] rounded-xl border border-slate-200 dark:border-slate-700" type="application/pdf" /></div>)}
                                    <div className="prose prose-sm max-w-none text-slate-700 dark:text-slate-300 leading-relaxed max-h-[400px] overflow-y-auto p-2 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700"><SimpleMarkdown text={generalInput?.extractedText || generalInput?.content || ''} /></div>
                                </div>
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                        {GENERAL_METHODS.map(m => {
                                            ...
                                            return (<button key={m.id} onClick={() => onStartGeneralSession(m.id)} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-500 hover:shadow-md transition text-left flex flex-col h-full"><div className="mb-3 text-indigo-600 dark:text-indigo-400"><Icon size={24}/></div><h4 className="font-bold text-sm mb-1 text-slate-900 dark:text-white">{m.name}</h4><p className="text-xs text-slate-500 dark:text-slate-400">{m.desc}</p></button>);
                                        })}
                                    </div>
                                </div>
                            </div>
                ...
                {category === 'UTBK' && (<><button onClick={() => onStartSession(StudyMode.SIMULATION)} className="w-full bg-slate-900 dark:bg-indigo-900 text-white p-6 rounded-2xl mb-8 flex justify-between items-center group shadow-lg shadow-indigo-200 dark:shadow-none"><div><h3 className="text-xl font-bold flex items-center gap-2"><Clock size={24} className="text-rose-400"/> Simulasi UTBK SNBT Full</h3><p className="text-slate-400 text-sm mt-1">155 Soal. Durasi 195 Menit.</p></div><ChevronRight className="group-hover:translate-x-2 transition-transform"/></button>{renderSubtests(UTBK_SUBTESTS)}</>)}
                {['TPA', 'PSIKOTEST'].includes(category) && (<><button onClick={() => onStartSession(StudyMode.SIMULATION)} className="w-full bg-slate-900 dark:bg-indigo-900 text-white p-6 rounded-2xl mb-8 flex justify-between items-center group"><div><h3 className="text-xl font-bold flex items-center gap-2"><Brain size={24} className="text-indigo-400"/> Simulasi {category} Full</h3><p className="text-slate-400 text-sm mt-1">{category === 'TPA' ? '65 Soal. Durasi 100 Menit.' : 'Simulasi Lengkap dengan Timer'}</p></div><ChevronRight className="group-hover:translate-x-2 transition-transform"/></button>{renderSubtests(category === 'TPA' ? TPA_SUBTESTS : PSIKOTEST_SUBTESTS)}</>)}
                {category === 'INTERVIEW' && renderSubtests(INTERVIEW_TOPICS)}
                {category === 'SKD' && (!skdStream ? (<div className="grid md:grid-cols-2 gap-6 mb-8"><button onClick={() => onSkdStreamSelect('KEDINASAN')} className="bg-white dark:bg-slate-800 p-8 rounded-2xl border-2 border-slate-200 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-500 transition text-center group"><Landmark size={48} className="mx-auto mb-4 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition"/><h3 className="text-xl font-bold text-slate-800 dark:text-white">Sekolah Kedinasan</h3></button><button onClick={() => onSkdStreamSelect('CPNS')} className="bg-white dark:bg-slate-800 p-8 rounded-2xl border-2 border-slate-200 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-500 transition text-center group"><Building2 size={48} className="mx-auto mb-4 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition"/><h3 className="text-xl font-bold text-slate-800 dark:text-white">CPNS Umum</h3></button></div>) : (<><div className="flex items-center gap-2 mb-4 text-sm text-slate-600 dark:text-slate-300"><button onClick={() => onSkdStreamSelect(null as any)} className="underline">Ubah Jalur</button> <ChevronRight size={14}/> <b>{skdStream}</b></div><button onClick={() => onStartSession(StudyMode.SIMULATION)} className="w-full bg-slate-900 dark:bg-indigo-900 text-white p-6 rounded-2xl mb-8 flex justify-between items-center shadow-lg shadow-indigo-200 dark:shadow-none"><div><h3 className="text-xl font-bold flex items-center gap-2"><Clock size={24} className="text-rose-400"/> Simulasi SKD CAT</h3><p className="text-slate-400 text-sm mt-1">TWK, TIU, TKP. Durasi 100 Menit.</p></div><ChevronRight className="group-hover:translate-x-2 transition-transform"/></button>{renderSubtests(SKD_SUBTESTS)}</>))}
                {category !== 'GENERAL' && category !== 'INTERVIEW' && category !== 'SKRIPSI' && (
                    <div className="mt-8 animate-fade-in-up delay-100">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <button onClick={() => onStartSession(StudyMode.DRILL)} className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:shadow-md transition text-left"><Zap className="text-amber-500 mb-2"/><div className="font-bold text-sm text-slate-800 dark:text-white">Learn & Drill</div></button>
                            <button onClick={() => onStartSession(StudyMode.ACTIVE_RECALL)} className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:shadow-md transition text-left"><Brain className="text-emerald-500 mb-2"/><div className="font-bold text-sm text-slate-800 dark:text-white">Active Recall</div></button>
                            {weakTopicsCount > 0 && (<button onClick={() => onStartSession(StudyMode.WEAKNESS)} className="p-4 bg-white dark:bg-slate-800 border border-rose-200 dark:border-rose-900 rounded-xl hover:border-rose-500 hover:shadow-md transition text-left"><Target className="text-rose-500 mb-2"/><div className="font-bold text-sm text-slate-800 dark:text-white">Weakness Attack</div></button>)}
                        </div>
                    </div>
                )}
            </>
